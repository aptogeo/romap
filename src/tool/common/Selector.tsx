                ...
                <Container className={className}>
                    ...
                    >
                        <option value="">{context.getLocalizedText('selector.type', 'Select type')}</option>
                        {
                            props.selectorTypes.map(selectorType => {
                                return (<option key={selectorType.type} value={selectorType.type}>{selectorType.description}</option>);
                            })
                        }
                    </select>
                    {selectorType && selectorType.showFileDropZone == true && (
                        <div onClick={() => inputFileRef.current.click()}>
                            ...
                            />
                            <DropZone><DropZoneText>{context.getLocalizedText('selector.dropzone', 'Drop file here or click to upload.')}</DropZoneText></DropZone>
                        </div>
                    )}
